...
impl ReportableError for DbError {
    ...
    fn is_sentry_event(&self) -> bool {
        match &self.kind {
            ...
            DbErrorKind::Pg(e) => e.is_sentry_event(),
        }
    }
    ...
    fn metric_label(&self) -> Option<String> {
        match &self.kind {
            ...
            DbErrorKind::Pg(e) => e.metric_label(),
        }
    }
...
from_error!(
    diesel::result::Error,
    DbError,
    |error: diesel::result::Error| DbError::from(DbErrorKind::Pg(SqlError::from(error)))
);
from_error!(
    diesel::result::ConnectionError,
    DbError,
    |error: diesel::result::ConnectionError| DbError::from(DbErrorKind::Pg(SqlError::from(
        error
    )))
);
from_error!(
    ...
    DbError,
    |error: diesel::r2d2::PoolError| DbError::from(DbErrorKind::Pg(SqlError::from(error)))
);
from_error!(
    diesel_migrations::RunMigrationsError,
    DbError,
    |error: diesel_migrations::RunMigrationsError| DbError::from(DbErrorKind::Pg(
        SqlError::from(error)
    ))
);
