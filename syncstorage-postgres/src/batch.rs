...
pub fn create(db: &PgDb, params: params::CreateBatch) -> DbResult<results::CreateBatch> {
...
pub fn validate(db: &PgDb, params: params::ValidateBatch) -> DbResult<bool> {
...
pub fn append(db: &PgDb, params: params::AppendToBatch) -> DbResult<()> {
    let exists = validate(
        db,
...
pub fn get(db: &PgDb, params: params::GetBatch) -> DbResult<Option<results::GetBatch>> {
...
pub fn delete(db: &PgDb, params: params::DeleteBatch) -> DbResult<()> {
    let batch_id = decode_id(&params.id)?;
    let user_id = params.user_id.legacy_id as i64;
    let collection_id = db.get_collection_id(&params.collection)?;
...
pub fn commit(db: &PgDb, params: params::CommitBatch) -> DbResult<results::CommitBatch> {
    let batch_id = decode_id(&params.batch.id)?;
    let user_id = params.user_id.legacy_id as i64;
    let collection_id = db.get_collection_id(&params.collection)?;
